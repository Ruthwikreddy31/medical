...
export default function Register({ onModeChange }) {
  ...
  async function submit(event) {
    ...
    const validationError = validateAuthForm(form, "register");
    ...
    <AuthCard title="Create Account">
      <form className="space-y-4" onSubmit={submit}>
        ...
        <button className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-clinic-green px-4 py-3 font-semibold text-white disabled:bg-slate-400" disabled={loading} type="submit">
          {loading ? <Loader2 className="animate-spin" size={19} /> : <UserPlus size={19} />}
          Register
        </button>
      </form>
      ...
      <div className="space-y-2">
        {!import.meta.env.VITE_GOOGLE_CLIENT_ID && (
          <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-3 mb-3 leading-relaxed">
            ⚠️ <strong>Google Client ID is not configured.</strong> Real Google registration requires setting <code>VITE_GOOGLE_CLIENT_ID</code> in <code>frontend/.env</code>.<br className="mb-1" />For local testing, please click the <strong>Dev Mock Google Login</strong> button below.
          </div>
        )}
      ...
      <button className="mt-4 w-full rounded-md border border-clinic-line px-4 py-2 text-sm font-semibold text-clinic-ink hover:border-clinic-green" onClick={() => onModeChange("login")} type="button">
